...
La proportion de transactions frauduleuses est configurée comme paramètre direct du simulateur en fixant les probabilités de déclenchement des comportements frauduleux pour chaque agent fraudeur. La fourchette cible retenue est $[20\%,\, 26\%]$, conformément aux configurations validées par \citet{azamuke2024momtsim, azamuke2025rich} sur les jeux MoMTSim documentés en section~1.7.4  les datasets publiés MoMTSim~202307 et MoMTSim~202309 présentent respectivement $25{,}9\%$ et $27{,}2\%$ de transactions frauduleuses \citep{azamuke2025dataset}. Cette fourchette résulte d'un arbitrage entre deux contraintes opposées :
...
Le déséquilibre résiduel de l'ordre de $1:4$ (fraude:légitime) produit par la configuration MoMTSim est géré au niveau de la fonction de perte par une pondération inversement proportionnelle à la fréquence de chaque classe, sans aucun rééchantillonnage additionnel qui compromettrait la cohérence temporelle des séquences générées par MoMTSim. Les formules de pondération et la fonction de perte pondérée sont détaillées en section~\ref{subsubsec:perte-predictive}, dans le cadre de la conception complète de la fonction de perte régularisée de MKAN.
...
\subsection{Adaptation du protocole SSE à la calibration des scénarios de fraude par SPSA}

Le protocole SSE de référence (section~3.1.3) indexe les distributions sur les types de transaction $\mathcal{C} = \{\text{CASH-IN},\ \text{CASH-OUT},\ \text{PAYMENT},\ \text{TRANSFER},\ \text{DEBIT}\}$ et compare à une distribution empirique issue de données réelles. Ce mémoire adapte ce protocole à un objectif distinct : calibrer automatiquement les probabilités de déclenchement des cinq scénarios de fraude de sorte que le taux de fraude global reste dans la fourchette cible $[20\%,\, 26\%]$ établie en section~3.1.1.

\paragraph{Espace de paramètres.}
Le vecteur de paramètres à calibrer est :
\begin{equation}
    \theta = \left(p_\text{ATO},\ p_\text{REFUND},\ p_\text{FAKE\_CRED},\ p_\text{SPLIT\_DEP},\ \alpha_\text{SMURFING}\right) \in \Theta \subset \mathbb{R}^5_+
\end{equation}
où $p_c \in [10^{-4},\, 0{,}5]$ est la probabilité de déclenchement du scénario $c$ à chaque pas de simulation, et $\alpha_\text{SMURFING} \in [0{,}1,\, 10]$ est un multiplicateur de fréquence pour la chaîne de mules dont la logique interne fixe elle-même le rythme des opérations. La borne inférieure $10^{-4}$ est cohérente avec les taux de fraude empiriques : \citet{gsma2024fraud} rapportent une perte moyenne de $\approx 0{,}03\,\%$ du volume transactionnel des opérateurs Mobile Money (enquête sur 34 pays), et le simulateur PaySim de \citet{lopezrojas2016paysim}, calibré sur des données réelles d'un opérateur africain, présente un taux de $0{,}13\,\%$  ces valeurs ($\sim 10^{-4}$ à $10^{-3}$) justifient la borne inférieure retenue. La borne supérieure $0{,}5$ délimite l'espace d'exploration de SPSA et ne correspond à aucun régime opérationnel observé. Pour $\alpha_\text{SMURFING}$, l'intervalle $[0{,}1,\, 10]$ couvre la plage d'activité documentée des chaînes de structuration en Mobile Money \citep{zhdanova2014smurfs} et est compatible avec les analyses récentes qui identifient typiquement entre 2 et 10 participants par chaîne de blanchiment \citep{deprez2025gargaml}.

\paragraph{Ensemble d'indexation et distribution cible.}
L'ensemble d'indexation est redéfini comme :
\begin{equation}
    \mathcal{C}' = \left\{\text{ATO},\ \text{REFUND},\ \text{FAKE\_CRED},\ \text{SPLIT\_DEP},\ \text{SMURFING}\right\}
\end{equation}
La construction de la distribution cible $D_r(c, t)$ repose sur deux choix justifiés. D'abord, le taux de fraude médian $r_\text{cible} = 0{,}23$ est le point central de la fourchette $[20\%,\, 26\%]$ établie en section~3.1.1 ; la cible en volume de transactions frauduleuses au pas $t$ est donc $N_\text{lég}(t) \cdot r_\text{cible} / (1 - r_\text{cible})$. Le volume légitime $N_\text{lég}(t)$ n'est pas uniforme : il est tiré du profil horaire \texttt{step\_target\_count} de MoMTSim, calibré par \citet{azamuke2024momtsim} sur des données réelles de transactions Mobile Money d'Afrique subsaharienne, qui reflète les variations intra-journalières et intra-hebdomadaires du volume observées en pratique (pics aux heures de marché, creux nocturnes). Ensuite, ce volume est réparti équitablement entre les cinq scénarios, conformément au principe d'équirépartition retenu en section~3.1.1 et validé par \citet{azamuke2022scenario} qui montrent que l'équilibre inter-scénarios est une condition nécessaire pour que chaque typologie de fraude soit représentée en quantité suffisante pour l'apprentissage supervisé :
\begin{equation}
    D_r(c, t) = \frac{N_\text{lég}(t) \cdot r_\text{cible}}{(1 - r_\text{cible}) \cdot |\mathcal{C}'|}, \quad \forall c \in \mathcal{C}'
\end{equation}

\paragraph{Moyennage stochastique sur plusieurs réplications.}
MoMTSim est une simulation stochastique à base d'agents : deux exécutions avec des paramètres identiques $\theta$ produisent des réalisations différentes de $D_s(c,t;\theta)$ en raison des tirages aléatoires internes. Évaluer l'objectif sur une seule réplication introduit un bruit de variance élevé qui perturbe le signal du gradient approché. \citet{law2000simulation} établissent que l'estimateur moyen sur $K$ réplications indépendantes réduit la variance de l'objectif d'un facteur $K$, et recommandent $K \in [3, 10]$ comme plage permettant un compromis entre réduction de variance et coût de calcul. Ce mémoire retient $K = 3$ réplications, chacune exécutée avec une graine aléatoire distincte $\xi_1, \xi_2, \xi_3$ :
\begin{equation}
    \bar{D}_s(c, t;\, \theta) = \frac{1}{3} \sum_{k=1}^{3} D_s(c, t;\, \theta,\, \xi_k)
\end{equation}
L'objectif à minimiser est alors :
\begin{equation}
    \text{sse}(\theta) = \sum_{c \in \mathcal{C}'} \sum_{t=1}^{T} \left(D_r(c, t) - \bar{D}_s(c, t;\, \theta)\right)^2
\end{equation}

\paragraph{Optimisation par SPSA.}
Les simulations à base d'agents sont des systèmes stochastiques à événements discrets non différentiables au sens classique : l'espace d'état est discret, les transitions sont discontinues, et la fonction $\theta \mapsto \text{sse}(\theta)$ n'admet pas de gradient analytique \citep{fu2002simulation}. Les méthodes de descente de gradient standard et les différences finies, qui requièrent $2|\theta|$ évaluations par itération pour estimer le gradient complet, sont de ce fait inapplicables ou trop coûteuses.

L'algorithme \textit{Simultaneous Perturbation Stochastic Approximation} (SPSA), introduit par \citet{spall1992spsa}, résout ce problème en estimant le gradient approché à partir de seulement deux évaluations de la fonction objectif, quelle que soit la dimension du vecteur $\theta$. La perturbation est simultanée sur toutes les composantes : à chaque itération $k$, un vecteur aléatoire $\delta_k$ est tiré selon une distribution de Rademacher symétrique, et le gradient approché est construit à partir de la différence de l'objectif aux deux points perturbés :
\begin{align}
    \delta_k &\overset{\text{i.i.d.}}{\sim} \mathcal{U}\!\left(\{-1,+1\}^5\right) \\[4pt]
    \hat{g}_k &= \frac{\text{sse}\!\left(\theta_k + c\,\delta_k\right) - \text{sse}\!\left(\theta_k - c\,\delta_k\right)}{2\,c\,\delta_k}
\end{align}
où la division est élément par élément, et $c = 0{,}02$ est l'amplitude de perturbation. La mise à jour du paramètre est :
\begin{equation}
    \theta_{k+1} = \Pi_\Theta\!\left(\theta_k - \alpha\,\hat{g}_k\right)
\end{equation}
où $\alpha = 0{,}05$ est le pas d'apprentissage et $\Pi_\Theta$ est la projection sur le domaine admissible $\Theta$ qui garantit que les contraintes de borne $\theta \in [10^{-4},\, 0{,}5]^4 \times [0{,}1,\, 10]$ sont respectées à chaque itération. \citet{spall1992spsa} démontrent la convergence de cet estimateur vers le gradient réel sous des conditions de régularité faibles compatibles avec les simulations bruitées, et établissent que son efficacité asymptotique est identique à celle des méthodes de différences finies tout en réduisant le coût par itération d'un facteur $|\theta| = 5$.

La meilleure configuration rencontrée, $\theta^* = \arg\min_{k \leq K_\text{max}} \text{sse}(\theta_k)$, est retenue et sauvegardée dans \texttt{calibrated\_probas.json} pour utilisation dans les simulations de production.

\section{Correspondance entre taxonomie de fraude et scénarios simulables}
\label{sec:scenarios}

MoMTSim est un environnement de simulation multi-agents dont l'architecture repose sur la définition de profils comportementaux pour chaque type d'agent : client légitime, client fraudeur, marchand, agent distributeur, banque. L'injection d'un scénario de fraude dans MoMTSim consiste à définir un profil d'agent fraudeur spécifique, à paramétrer ses règles de décision transactionnelle, et à le déployer dans l'environnement aux côtés des agents légitimes. Cette section détaille la configuration technique des cinq scénarios retenus. Le tableau~\ref{tab:scenarios_momtsim} en offre une vue synoptique avant le détail de chaque configuration.
...
\multicolumn{1}{c}{\textbf{Scénario}}
  & \multicolumn{1}{c}{\textbf{Type d'agent fraudeur}}
  & \multicolumn{1}{c}{\textbf{Transactions générées}}
  & \multicolumn{1}{c}{\textbf{Fenêtre temporelle}}
  & \multicolumn{1}{c}{\textbf{Feature discriminante principale}} \\
\midrule
ATO
  & Client compromis
  & TRANSFER /\newline CASH-OUT
...
Split Deposit
  ...
  & CASH-IN multiple
  & 60 -- 120~sec
  & Agrégation au niveau du guichet agent \\[4pt]
Smurfing
  & Réseau $f_1$ + mules + $f_2$
  & TRANSFER chaîné
  & Mensuel
  & Critères relationnels \citet{zhdanova2014smurfs} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Destinataires :} les comptes destinataires sont exclusivement des comptes avec lesquels le compte victime ne présente aucune transaction antérieure dans l'historique simulé, garantissant que la feature \og nouveauté du destinataire \fg{} est systématiquement activée.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Registre des marchands vulnérables :} l'agent fraudeur est initialisé avec un registre de marchands cibles $\mathcal{M}_{vuln} \subseteq \mathcal{M}$ sélectionnés parmi les agents marchands du simulateur selon leur probabilité d'acceptation de remboursement $p_{refund}(m)$, fixée à $p_{refund}(m) > 0{,}7$ pour les marchands vulnérables.
...
La modélisation de la Fake Credentials Fraud exploite directement la richesse du modèle agent de MoMTSim qui intègre les attributs d'identité NIN, adresse électronique et numéro de téléphone, attributs absents de PaySim. La configuration technique consiste à créer des agents fraudeurs dont le profil d'identité est partiellement usurpé et partiellement synthétique.

La procédure d'injection est la suivante :

\begin{itemize}
    \item \textbf{Construction de l'identité hybride :} pour chaque agent Fake Credentials créé, le NIN est copié depuis un compte client légitime existant dans le simulateur, tandis que l'adresse électronique est générée selon le format \texttt{prenom.nom\_aleatoire@domaine\_fictif.cm} et le numéro de téléphone est généré dans la plage des préfixes MTN/Orange camerounais (\texttt{6[57-59|70-79|9X]XXXXXXX}) sans correspondance avec un compte réel.
...
\begin{itemize}
    \item \textbf{Float initial de l'agent distributeur :} conformément au mécanisme documenté par le GSMA \citep{gsma2010agents} et l'IFC \citep{ifc2017liquidity}, chaque agent distributeur est initialisé avec un stock de monnaie électronique (e-float\footnote{L'e-float désigne la réserve de valeur électronique prépayée détenue par un agent distributeur auprès de l'opérateur, lui permettant de créditer instantanément les comptes clients lors des opérations de dépôt.}) acheté à l'avance auprès de l'opérateur. Ce float initial est tiré selon une distribution à trois paliers reflétant la réalité du marché CEMAC : petits agents de proximité ($100\,000$--$500\,000$~FCFA, 60~\% du réseau), agents moyens ($500\,000$--$2\,000\,000$~FCFA, 30~\%), et grands agents ou supragents ($2\,000\,000$--$10\,000\,000$~FCFA, 10~\%). La contrainte de solde $B_{agent} \geq M$ est vérifiée avant l'exécution du split : si le float disponible est insuffisant, la fraude n'est pas tentée, ce qui est cohérent avec le fait qu'environ 20~\% des transactions sont refusées faute d'e-float suffisant selon \citet{cogent2023agent}.
    ...
    \item \textbf{Compte client fixe :} toutes les transactions du split sont créditées sur le même compte client destinataire, faisant du compte agent la seule entité à partir de laquelle la fraude est détectable par agrégation.
\end{itemize}

Ce scénario génère des séquences de 2 à 5 transactions \textsc{Cash-In} consécutives étiquetées frauduleuses, détectables uniquement par agrégation au niveau du guichet agent sur une fenêtre de 2 minutes, et non au niveau de chaque transaction individuelle.

\subsection{Scénario 5 : Smurfing, micro-structuration via réseau de mules}

Le scénario de smurfing est le plus complexe à injecter dans MoMTSim car il requiert la création et la coordination de plusieurs types d'agents distincts interagissant selon une topologie de graphe prédéfinie. La configuration s'appuie directement sur la formalisation de \citet{zhdanova2014smurfs} établie au Chapitre~1.

La configuration technique du graphe de fraud chain comprend :

\begin{itemize}
    \item \textbf{Création du réseau :} pour chaque fraud chain injectée, on crée un agent émetteur $f_1$, un agent récepteur $f_2$, et $n \sim \mathcal{U}(4,\, 10)$ agents mules $\{m_1, \ldots, m_n\}$. Les mules conscientes ($60\%$ du réseau) ont leurs règles de transfert entièrement contrôlées par $f_1$ ; les mules inconscientes ($40\%$) sont des comptes légitimes existants dont les identifiants ont été compromis, modélisant des victimes de hameçonnage qui transfèrent les fonds sans en comprendre la nature.
...
\section{Ingénierie des caractéristiques transactionnelles pour le flux haute fréquence}
...
\subsection{Features spécifiques aux scénarios de fraude}
...
\end{equation}
Cette feature est adaptée des variables de contrôle comptable introduites par \citet{lopezrojas2016paysim} dans le simulateur PaySim, premier travail à formaliser $\Delta B_{orig}$ comme signal de détection dans un contexte Mobile Money synthétique. L'adaptation consiste ici à exploiter la propriété de conservation comptable propre au contexte CEMAC : contrairement aux systèmes bancaires classiques où des frais variables peuvent expliquer un écart, les opérateurs Mobile Money en zone CEMAC ne prélèvent pas de commission sur le solde émetteur transaction par transaction, de sorte que l'identité $\Delta B_{orig} = \text{amount}$ est une contrainte stricte vérifiable. Tout écart significatif entre $\Delta B_{orig}$ et $\text{amount}$ constitue un signal d'incohérence comptable détectable, notamment dans le scénario Insider Fraud où un employé crée du flottant fictif sans contrepartie de dépôt réel. Cette feature est calculée directement à partir des variables brutes disponibles dans MoMTSim sans nécessiter d'historique.
...
\end{equation}
Également inspirée de \citet{lopezrojas2016paysim}, cette feature symétrique est ici adaptée au rôle spécifique du destinataire dans les typologies de fraude CEMAC : alors que PaySim traite $\Delta B_{dest}$ comme un contrôle de cohérence générique, ce travail l'exploite en tant que mesure directe de la commission de mule dans le scénario Smurfing, où la mule ne reçoit qu'une fraction de ce que l'émetteur a envoyé. La comparaison du couple $(\Delta B_{orig},\ \Delta B_{dest})$ avec $\text{amount}$ permet de quantifier simultanément la sortie de fonds chez l'émetteur et l'entrée nette chez le destinataire, révélant toute capture intermédiaire de valeur. Ce couple constitue un vecteur de contrôle de conservation des fonds exploitable nativement par les nœuds multiplicatifs de MultKAN.
...
\end{equation}
où $\epsilon = 10^{-6}$ est un terme de régularisation évitant la division par zéro, est une contribution originale de ce travail. Sa construction repose sur l'observation que le montant absolu d'une transaction est un signal faible en contexte Mobile Money : un retrait de 50\,000 FCFA est anodin pour un compte à 5\,000\,000 FCFA mais constitue un vidage total pour un compte à 50\,000 FCFA. En normalisant le montant par le solde \emph{avant} la transaction, $r_1$ produit une mesure adimensionnelle de la proportion du solde initial consommée, invariante à l'échelle du compte. Ce ratio est la feature principale pour la détection de l'Account Takeover (ATO, cf.\ section~\ref{sec:scenarios}) : une extraction frénétique caractéristique de ce scénario produit $r_1 \rightarrow 1$ sur chaque transaction de la séquence, tandis qu'une transaction légitime typique produit $r_1 \ll 1$. La distribution de $r_1$ est bornée dans $[0, 1]$, propriété favorable à la calibration des splines B qui opèrent sur un domaine compact.
...
\end{equation}
est également une contribution originale de ce travail, complémentaire à $r_1$. Alors que $r_1$ mesure la proportion du solde \emph{avant} la transaction, $r_2$ évalue l'impact sur le solde \emph{résiduel} : lorsque $r_2 \gg 1$, la transaction a consommé l'intégralité du solde disponible, voire produit un découvert, signal caractéristique de la phase terminale d'un ATO où le fraudeur vide le compte jusqu'au dernier centime. Le couple $(r_1, r_2)$ est conçu pour alimenter nativement un nœud multiplicatif MultKAN : leur produit $r_1 \cdot r_2$ amplifie le signal de vidage total et produit une règle analytique directement auditable par un superviseur COBAC sous la forme \og la transaction consomme une fraction $r_1$ du solde initial et laisse un solde résiduel tel que $r_2 > \tau$ \fg{}, où $\tau \in \mathbb{R}_+$ est un seuil de décision fixé par validation croisée (typiquement $\tau = 1$, signalant que le montant transféré excède le solde résiduel post-transaction).

\paragraph{Indicateur de transaction anormalement grande.}
La variable binaire :
\begin{equation}
    \text{Flag}_{anomalie} = \mathbf{1}\left[\text{amount} > \mu_{\text{amount}} + 2\sigma_{\text{amount}}\right]
\end{equation}
où $\mu_{\text{amount}}$ et $\sigma_{\text{amount}}$ sont la moyenne et l'écart-type des montants calculés sur les transactions du même type dans la fenêtre glissante des 10 dernières opérations du compte, applique le critère classique de détection d'outlier à $2\sigma$ \citep{bolton2002statistical} à l'analyse comportementale par compte. L'adaptation par rapport à l'usage standard en détection de fraude bancaire \citep{bolton2002statistical} consiste à calculer $\mu_{\text{amount}}$ et $\sigma_{\text{amount}}$ sur une fenêtre glissante \emph{par compte et par type de transaction} plutôt que sur la population globale : cette individualisation du seuil tient compte du profil transactionnel propre à chaque utilisateur Mobile Money, dont les habitudes de montant sont très hétérogènes dans le contexte CEMAC. Cette feature est transversale à plusieurs scénarios : elle est activée par la transaction d'exfiltration finale d'un Fake Credentials, par les montants anormalement élevés d'un ATO sur un compte habituellement peu actif, et par les transactions de la phase active du smurfing lorsque le montant total $X$ à blanchir est élevé relativement aux habitudes du compte émetteur.
...
\end{equation}
avec la contrainte de qualification $0 < \delta_{commission} \leq 0{,}10 \cdot a_{rec}$, correspondant au Critère~1 de \citet{zhdanova2014smurfs} formalisé au Chapitre~1. Le seuil de $10\%$ est retenu tel quel depuis \citet{zhdanova2014smurfs}, dont l'étude empirique sur les réseaux de mules bancaires européens établit que la commission de mule excède rarement $10\%$ du montant transféré, au-delà duquel le différentiel devient trop visible pour échapper aux contrôles de premier niveau. Cette valeur est conservée sans modification car les entretiens documentés au Chapitre~1 sur le contexte camerounais confirment une structure de commission analogue pour le Mobile Money. Cette feature est calculée en appariant, pour chaque compte, la transaction \textsc{Transfer} entrante la plus récente avec la transaction \textsc{Transfer} sortante la plus proche temporellement. Un compte présentant systématiquement $\delta_{commission}$ dans l'intervalle $]0,\ 0{,}10 \cdot a_{rec}]$ est identifié comme mule candidate. La normalisation $\delta_{commission} / a_{rec}$ produit une variable bornée dans $[0,\ 0{,}10]$ dont la distribution régulière est favorable à l'approximation par splines.
...
\end{equation}
Cette feature est une contribution originale de ce travail, motivée par la logique frauduleuse du Split Deposit : le fraudeur découpe un dépôt unique en $k$ fragments proches pour rester sous les seuils de déclaration, ce qui produit une distribution de montants quasi-constante — et donc une variance quasi-nulle — sur la fenêtre d'observation. La variance est délibérément calculée au niveau du \emph{guichet agent}, et non du compte client, car c'est l'agent qui orchestre le découpage : un même client pourrait légitimement effectuer plusieurs dépôts de même montant dans des agences différentes, mais un même agent réalisant plusieurs \textsc{Cash-In} de montant quasi-identique vers un même destinataire sur 2 minutes est un signal univoque. Une variance faible $(\text{Var}_{agent} \approx 0)$ couplée à $k \geq 2$ constitue ainsi la signature caractéristique du Split Deposit, inaccessible depuis une analyse centrée sur le compte client seul.
...
\end{equation}
où $\bar{m}_{historique}$ est la moyenne des montants des transactions antérieures du compte sur les 30 derniers jours et $\epsilon = 10^{-6}$ un terme de régularisation. Cette feature est une contribution originale de ce travail, conçue pour capturer la dynamique temporelle propre au scénario Fake Credentials : un compte frauduleux est créé avec de vraies données personnelles, utilisé de façon anodine pendant une longue période de dormance (transactions de faible montant pour établir un historique crédible), puis exploité pour une exfiltration massive unique. Le ratio $\rho_{rupture}$ formalise cette rupture : pour un compte légitime, les montants restent statistiquement proches de la moyenne historique ($\rho_{rupture} \approx 1$) ; pour un compte Fake Credentials en phase d'exfiltration, $\rho_{rupture} \gg 1$, signal amplifié par la longue phase de dormance qui établit un $\bar{m}_{historique}$ particulièrement bas. La fenêtre de 30 jours est un paramètre de conception retenu pour couvrir la durée minimale de dormance observée dans les typologies documentées par \citet{gsma2024fraud}.
...
\end{equation}
Cette feature est une contribution originale de ce travail, fondée sur la structure opérationnelle du scénario Refund Fraud en Mobile Money : le fraudeur effectue un \textsc{Payment} vers un marchand complice, puis déclenche un \textsc{Refund} du même montant, répétant ce cycle sur une période suffisamment longue pour que chaque opération individuelle reste sous les seuils d'alerte. Le ratio $\rho_{refund}$ formalise cette symétrie abusive : un compte légitime présente $\rho_{refund} \approx 0$ car les remboursements sont exceptionnels dans un usage normal du Mobile Money, tandis qu'un agent Refund Fraud produit $\rho_{refund} \rightarrow 1$ par construction du cycle \textsc{Payment}--\textsc{Refund}. La fenêtre de 30 jours est alignée sur les cycles de reporting mensuel imposés aux marchands par le cadre réglementaire COBAC, rendant cette feature directement interprétable pour un audit de conformité.

\subsection{Features transversales de contexte}
...
\end{equation}
La vélocité transactionnelle est une feature de référence en détection de fraude, introduite par \citet{fawcett1997adaptive} dans le contexte de la fraude téléphonique et généralisée depuis à la fraude bancaire \citep{bolton2002statistical}. L'adaptation ici porte sur deux dimensions : premièrement, la fenêtre temporelle est fixée à 1 heure plutôt que la journée habituellement retenue en banque, en cohérence avec la dynamique d'exfiltration rapide documentée pour le Mobile Money CEMAC \citep{gsma2024fraud} où un ATO se déroule typiquement en moins de 30 minutes ; deuxièmement, la granularité est le compte émetteur (\texttt{nameOrig}) et non la carte ou le terminal, unité d'observation naturelle dans les systèmes Mobile Money. En production temps réel, ce compteur glissant est maintenu en mémoire et mis à jour à chaque nouvelle transaction entrante, permettant un scoring en quelques millisecondes sans recalcul sur l'historique complet.
...
\begin{equation}
    \text{Flag}_{nuit} = \mathbf{1}\left[\text{step} \bmod 24 \in [22\text{h},\ 6\text{h}]\right]
\end{equation}
Cette feature est une contribution originale de ce travail, ancrée dans les spécificités comportementales du Mobile Money camerounais. Le rapport GSMA \citet{gsma2024fraud} documente que les transactions frauduleuses — notamment les ATO et les Fake Credentials — sont surreprésentées entre 22h et 6h, plage horaire où la vigilance des agents et des systèmes de surveillance est réduite. La plage $[22\text{h},\ 6\text{h}]$ est retenue comme seuil de conception car elle couvre la quasi-totalité des transactions nocturnes observées dans le corpus de fraude analysé au Chapitre~1, tout en restant assez étroite pour ne pas englober les transactions légitimes tardives (21h--22h) fréquentes en fin de journée de travail. Cette variable binaire, couplée à un montant élevé via un nœud multiplicatif MultKAN, produit un signal composite particulièrement discriminant : une transaction de montant élevé en pleine nuit constitue une anomalie comportementale forte dans le contexte camerounais où l'activité Mobile Money légitime est concentrée sur les heures diurnes.
...
\end{equation}
où $\mathcal{H}_{dest}(c)$ est l'ensemble des destinataires historiques du compte $c$ sur les 90 derniers jours. Cette feature est une contribution originale de ce travail, inspirée des approches d'analyse de graphe de transactions en lutte anti-blanchiment \citep{deprez2025gargaml}, qui montrent que les réseaux de mules forment systématiquement de nouvelles arêtes dans le graphe de transactions. L'adaptation par rapport aux méthodes de graphe consiste à projeter cette information de topologie de réseau en une feature scalaire temps-réel calculable transaction par transaction, sans nécessiter la construction d'un graphe complet : $\rho_{nouveau}$ mesure la proportion de destinataires \emph{nouveaux} dans la fenêtre courante relativement à l'historique des 90 derniers jours. La fenêtre d'observation de 30 jours et l'historique de référence de 90 jours sont des paramètres de conception : 30 jours couvrent la phase d'exfiltration typique, et 90 jours établissent un historique suffisamment long pour que les destinataires habituels soient bien représentés. Cette feature détecte la fuite des fonds vers des mules : un compte ATO compromis transfère ses fonds vers des destinataires jamais contactés auparavant, produisant $\rho_{nouveau} \rightarrow 1$, et un compte smurfing émetteur présente le même pattern, ses transferts vers les mules constituant systématiquement de nouvelles relations.

\subsection{Alignement des features avec l'architecture MultKAN}

L'ensemble des features construites est conçu pour exploiter nativement les nœuds multiplicatifs de l'architecture MultKAN documentée en section~2.4.5. Les features dérivées par ratio, à savoir $r_1$, $r_2$, $\rho_{rupture}$, $\rho_{refund}$, $\rho_{nouveau}$ et $\delta_{commission}/a_{rec}$, sont des relations multiplicatives entre variables brutes que MultKAN peut apprendre directement sans ingénierie supplémentaire, et dont les nœuds de multiplication explicites produiront des règles analytiques de la forme $r_1 \cdot \text{Flag}_{nuit} > \tau$ directement auditables par un superviseur COBAC. Le tableau~\ref{tab:features_transactionnelles} synthétise l'ensemble des features retenues, leur formule de calcul, le scénario cible et la granularité de calcul.
...
\multicolumn{1}{c}{\textbf{Feature}}
  & \multicolumn{1}{c}{\textbf{Formule}}
  & \multicolumn{1}{c}{\textbf{Scénario cible}}
  & \multicolumn{1}{c}{\textbf{Granularité}} \\
\midrule
$\Delta B_{orig}$
...
$r_2$
  ...
  & Transaction \\
$\text{Flag}_{anomalie}$
  & $\mathbf{1}[\text{amount} > \mu + 2\sigma]$
...
$\rho_{rupture}$
  ...
  & Fenêtre 30j \\
$\rho_{refund}$
  & $\tfrac{|\{t\text{:REFUND}\}_{W_{30j}}|}{|\{t\text{:PAYMENT}\}_{W_{30j}}| + \varepsilon}$
  & Refund Fraud
  & Fenêtre 30j \\
$V_{1h}$
  ...
  & Fenêtre 1h \\
$\text{Flag}_{nuit}$
  & $\mathbf{1}[\text{step} \bmod 24 \!\in\! [22\text{h},\,6\text{h}]]$
  & Tous
  & Transaction \\
$\rho_{nouveau}$
...
Cette normalisation est indispensable pour deux raisons. Premièrement, les features dérivées en section~3.3 présentent des échelles hétérogènes : les ratios $r_1$, $r_2$ sont bornés dans $[0,\,1]$, la vélocité $V_{1h}$ peut atteindre plusieurs dizaines, et la variance agent $\text{Var}_{agent}(m_j)$ s'exprime en FCFA$^2$. Sans normalisation, les composantes principales de la PCA seraient dominées par les features à grande variance, masquant la structure géométrique réelle de l'espace. Deuxièmement, les B-splines du réseau KAN opèrent sur des domaines compacts : leur grille de nœuds est définie sur l'intervalle des valeurs d'entrée, et une normalisation préalable garantit que les grilles sont calibrées sur des domaines comparables pour toutes les features.
...
La première est l'absence de discontinuités majeures, évaluée par le test de Kolmogorov-Smirnov sur la distribution empirique de $\tilde{x}_j$ : une statistique $D_{KS} < 0{,}15$ indique une distribution suffisamment régulière pour être approximée efficacement par des B-splines de degré~3. Les features présentant $D_{KS} \geq 0{,}15$ sont candidates à une transformation préalable, à savoir une transformation logarithmique pour les distributions fortement asymétriques, ou une transformation de rang pour les distributions multimodales.
...
L'entrée de la cellule à chaque pas $t$ est le vecteur de features $\mathbf{x}_t \in \mathbb{R}^{d}$ construit en section~3.3, concaténé avec l'état caché précédent $h_{t-1}$. Les quatre portes de la cellule, à savoir la porte d'oubli $f_t$, la porte d'entrée $i_t$, le candidat $\tilde{c}_t$ et la porte de sortie $o_t$, sont calculées selon :
...
La composante gaussienne, issue de FastKAN \citep{Li2024}, apporte la vitesse d'inférence. Comme démontré par \citet{Li2024}, l'évaluation des fonctions à base radiale gaussiennes s'exprime entièrement comme des opérations d'algèbre linéaire matricielle parallélisables sur GPU, évitant les branchements conditionnels et les accès mémoire irréguliers de la récurrence de Cox-de Boor des B-splines. \citet{Li2024} documentent une accélération du passage avant de $3{,}33$ fois par rapport à \texttt{efficient\_kan} sur GPU NVIDIA V100, ce qui ramène la latence d'inférence dans la plage de quelques centaines de microsecondes compatible avec le scoring en ligne des transactions Mobile Money.

La composante de Fourier, issue de KAN-AD \citep{Zhou2025}, apporte la robustesse au bruit comportemental. Les fonctions sinusoïdales et cosinusoïdales possèdent une régularité globale infinie : une perturbation locale dans les données d'entraînement ne peut pas être absorbée par une expansion de Fourier de faible degré sans générer des oscillations globales pénalisées par la régularisation. \citet{Zhou2025} démontrent sur le benchmark TODS, dont les données d'entraînement contiennent une proportion substantielle d'anomalies dans une condition analogue aux données MoMTSim, une amélioration de $27\%$ en Event F1 par rapport à l'état de l'art, avec seulement 274 paramètres entraînables. Cette propriété est essentielle pour éviter que les patterns frauduleux présents dans les données d'entraînement soient incorporés dans la représentation du comportement normal, ce qui produirait des faux négatifs précisément là où la détection est critique.

Le nombre de termes est fixé à $M = 8$ centres gaussiens et $K = 2$ harmoniques de Fourier. Le choix de $K = 2$ s'appuie sur le résultat empirique de \citet{Zhou2025} qui identifient cette configuration comme optimale sur UCR, suffisante pour capturer les périodicités hebdomadaire et mensuelle des transactions Mobile Money documentées en section~1.1.5 sans introduire de surapprentissage par excès de termes.
...
où $\sigma$ est la fonction sigmoïde, $\mathbf{w}$ et $b$ sont les paramètres apprenables de la couche de projection. Le score $\hat{y}_t$ constitue la probabilité de fraude estimée pour la transaction $t$, exploitable directement par le cadre d'évaluation défini en section~\ref{sec:evaluation_framework} via les métriques MCC (éq.~\ref{eq:mcc}), AUC-ROC (éq.~\ref{eq:auc}), Précision (éq.~\ref{eq:precision}), Rappel (éq.~\ref{eq:rappel}), $F_1$ (éq.~\ref{eq:f1}), PR-AUC (éq.~\ref{eq:prauc}) et Score de Brier (éq.~\ref{eq:brier}).
...
L'originalité de MKAN ne réside pas dans l'utilisation isolée de l'une de ces variantes, chacune ayant été documentée individuellement en section~2.4, mais dans leur intégration structurelle au sein d'une cellule récurrente unifiée dont chaque arête porte une fonction hybride gaussienne-Fourier opérant sur des nœuds à interactions multiplicatives explicites. C'est cette intégration simultanée qui permet à MKAN de satisfaire conjointement des contraintes que chaque variante isolée ne peut satisfaire qu'individuellement.
...
\]
sont inversement proportionnels à la fréquence de chaque classe. Ce terme assure que les erreurs sur la classe minoritaire frauduleuse contribuent autant à la mise à jour des paramètres que les erreurs sur la classe majoritaire légitime.

\subsection{Terme de régularisation L1 pour la sparsification des arêtes}
\label{subsubsec:regularisation-l1}

Le second terme est la régularisation par norme L1 appliquée aux fonctions d'activation de chaque arête, telle que formalisée par \citet{liu2024kan} et établie en section~2.5.2. Dans MKAN, cette régularisation opère sur les fonctions d'arête hybrides gaussienne-Fourier définies en section~4.2.2. La norme L1 d'une fonction d'arête $\phi_{ij}$ est définie comme la moyenne de ses valeurs absolues sur l'ensemble des données d'entraînement :
...
La fonction de perte totale intègre les trois termes selon la formulation de \citet{liu2024kan} établie en section~2.5.2, adaptée à l'architecture MKAN :
...
Un neurone dont le score entrant $s_j^{in} < \theta$ ou le score sortant $s_j^{out} < \theta$ est supprimé, entraînant la suppression de toutes ses arêtes incidentes. Le seuil $\theta = 10^{-2}$ est retenu comme valeur initiale conformément aux recommandations de \citet{liu2024kan}. Ce processus est appliqué itérativement jusqu'à stabilisation du graphe, produisant le réseau élagué $\text{MKAN}^*$ dont les arêtes survivantes constituent les candidats à la régression symbolique.
...
\section{Stratégies heuristiques pour la configuration des hyperparamètres de MKAN}
\label{sec:heuristiques_hyperparams}

La configuration des hyperparamètres d'une architecture hybride telle que MKAN constitue un problème d'optimisation combinatoire de haute dimension, fondamentalement non convexe et non différentiable au sens des méthodes de gradient standard. Les hyperparamètres à configurer incluent : la taille de la grille B-spline $G \in \{5, 10, 20, 30\}$, le degré de la spline $k \in \{3, 4, 5\}$, le nombre de couches $L \in \{2, 3, 4\}$, la dimension de l'état caché T-KAN $h \in \{32, 64, 128\}$, la taille de la fenêtre temporelle $W \in \{5, 10, 20\}$, les coefficients de régularisation $(\lambda_1, \lambda_2, \lambda_{ent}) \in \mathbb{R}^3_+$ de la fonction de perte définie en section~4.3, et les paramètres du mécanisme de Grid Extension récapitulés au tableau~\ref{tab:grid_extension_params}. L'espace de recherche total dépasse $10^6$ combinaisons possibles, rendant l'exploration exhaustive computationnellement prohibitive dans un cadre de mémoire de master.

\subsection{Approches heuristiques classiques et leur applicabilité à MKAN}

Les méthodes heuristiques de configuration des hyperparamètres se distinguent des méthodes de descente de gradient par leur capacité à opérer sur des espaces discrets, non différentiables et bruités \citep{haosol2024meta}. Trois familles principales sont pertinentes pour MKAN.

\paragraph{Recherche sur grille (\textit{grid search}).}
La recherche sur grille consiste à évaluer exhaustivement toutes les combinaisons d'un sous-ensemble discret et prédéfini de valeurs pour chaque hyperparamètre. Pour MKAN, une grille réduite à trois valeurs par hyperparamètre sur les six paramètres primaires produit $3^6 = 729$ configurations, chacune nécessitant un entraînement complet sur les données MoMTSim. Cette approche garantit la couverture systématique de l'espace réduit mais souffre de la malédiction de la dimensionnalité : l'ajout de chaque hyperparamètre supplémentaire multiplie le coût par le nombre de valeurs testées pour ce paramètre. La recherche sur grille est retenue comme protocole de validation croisée sur les hyperparamètres primaires $(G, k, L)$, dont l'interaction avec les propriétés des B-splines est directement documentée dans \citet{liu2024kan} et \citet{somvanshi2025survey}.

\paragraph{Recherche aléatoire (\textit{random search}).}
La recherche aléatoire tire aléatoirement des configurations dans l'espace des hyperparamètres et évalue chacune sur un sous-ensemble de validation. \citet{haosol2024meta} établissent que la recherche aléatoire domine asymptotiquement la recherche sur grille lorsque tous les hyperparamètres n'ont pas une importance égale sur la performance finale : si seuls $k$ des $d$ hyperparamètres sont critiques, la recherche aléatoire couvre $k$ valeurs distinctes pour ces paramètres critiques sur $n$ configurations, contre $n/d^{d-k}$ pour la recherche sur grille. Dans le cas de MKAN, les hyperparamètres les plus critiques sont la taille de grille $G$ et le coefficient d'entropie $\lambda_{ent}$, comme établi par les analyses de sensibilité de \citet{hou2024critical} sur les architectures KAN : la recherche aléatoire est appliquée sur les hyperparamètres secondaires $(\lambda_1, \lambda_2, W)$ avec $n = 50$ configurations tirées uniformément dans leurs plages admissibles.

\paragraph{Heuristique de diagnostic architectural (\textit{Failure as Diagnosis}).}
La troisième approche est spécifique aux architectures KAN et constitue une contribution méthodologique du cadre \og Failure as Diagnosis \fg{} de \citet{hou2024critical} présenté en section~2.3.4. Plutôt que d'explorer l'espace des hyperparamètres de manière aveugle, cette heuristique utilise les patterns de défaillance observés pendant l'entraînement comme signal diagnostique orientant la reconfiguration. Une saturation de la perte d'entraînement au-dessus de $0{,}15$ après les 100 premières époques indique une grille trop grossière ($G$ sous-dimensionné) ; un écart trop important entre perte d'entraînement et perte de validation indique un surapprentissage corrigible par augmentation de $\lambda_1$ ou de $\lambda_{ent}$ ; une variance élevée des gradients des arêtes indique un taux d'apprentissage inadapté à la taille de la grille. Cette heuristique diagnostique réduit l'espace de recherche en excluant les directions de reconfiguration non pertinentes, et constitue le premier filtre appliqué avant toute optimisation systématique.

\subsection{Protocole de configuration retenu pour MKAN}

Le protocole de configuration adopté dans ce mémoire combine séquentiellement les trois heuristiques décrites ci-dessus selon une logique en trois phases inspirée de la méthodologie SPSA de calibration des scénarios de fraude documentée en section~3.1.4 \citep{spall1992spsa} : exploration diagnostique, recherche sur grille réduite, puis affinage aléatoire.

La \textbf{phase 1 --- diagnostic initial} consiste à entraîner MKAN sur un échantillon de $10\,000$ transactions avec la configuration de référence recommandée par \citet{liu2024kan} ($G = 10$, $k = 3$, $L = 2$, $h = 64$, $W = 10$), à analyser les courbes de convergence et les distributions des gradients par arête, et à identifier les hyperparamètres à ajuster selon le protocole \og Failure as Diagnosis \fg{} de \citet{hou2024critical}. Cette phase ne nécessite qu'un seul entraînement de quelques minutes et fournit un signal diagnostique directement exploitable.

La \textbf{phase 2 --- recherche sur grille réduite} consiste à évaluer les configurations retenues sur la grille $G \in \{10, 20, 30\}$, $k \in \{3, 4\}$, $L \in \{2, 3\}$ sélectionnée en fonction des diagnostics de la phase~1, en utilisant la validation croisée temporelle \textit{walk-forward} sur les données MoMTSim pour éviter toute contamination temporelle. La validation croisée temporelle \textit{walk-forward} est la seule procédure de validation admissible pour les séries transactionnelles : elle entraîne le modèle sur le passé et le teste sur le futur, reproduisant fidèlement les conditions de mise en production \citep{law2000simulation}.

La \textbf{phase 3 --- affinage aléatoire} consiste à affiner les hyperparamètres secondaires $(\lambda_1, \lambda_2, \lambda_{ent}, W, \tau_{drift})$ par $n = 50$ configurations tirées uniformément dans leurs plages admissibles, en fixant les hyperparamètres primaires à leurs valeurs optimales issues de la phase~2. La configuration finale $\theta^*_{MKAN}$ issue de ce protocole est évaluée sur le jeu de test MoMTSim hors-échantillon selon le cadre d'évaluation défini en section~\ref{sec:evaluation_framework}.

\section{Cadre d'évaluation des performances en environnement de classes asymétriques en conditions normales d'exploitation}
\label{sec:evaluation_framework}

La détection de fraude Mobile Money est caractérisée par un déséquilibre extrême des classes : les transactions frauduleuses représentent typiquement moins de 1\,\% du volume total dans les données MoMTSim. Dans ce contexte, les métriques classiques telles que l'exactitude globale sont inadaptées : un classifieur naïf qui prédit systématiquement \og légitime \fg{} atteint une exactitude de plus de 99\,\% sans détecter une seule fraude. L'évaluation de MKAN repose sur cinq métriques complémentaires adaptées à ce déséquilibre, calculées à seuil de décision $\tau = 0{,}5$ sauf pour les métriques de courbe. Ces métriques sont calculées en conditions normales d'exploitation, c'est-à-dire hors des périodes de dérive conceptuelle active traitées en section~4.4 : elles caractérisent la performance du modèle sur la distribution d'entraînement, avant activation du mécanisme de Grid Extension.

\subsection{Coefficient de corrélation de Matthews (MCC)}
\label{subsec:mcc}

Le Coefficient de Corrélation de Matthews (MCC) constitue la métrique principale d'évaluation et de comparaison avec la baseline XGBoost. Il intègre les quatre types de résultats possibles de la matrice de confusion, à savoir les Vrais Positifs (VP), Vrais Négatifs (VN), Faux Positifs (FP) et Faux Négatifs (FN), dans une seule valeur scalaire symétrique :

\begin{equation}
    \text{MCC} = \frac{\text{VP} \cdot \text{VN} - \text{FP} \cdot \text{FN}}
                      {\sqrt{(\text{VP}+\text{FP})(\text{VP}+\text{FN})(\text{VN}+\text{FP})(\text{VN}+\text{FN})}}
    \label{eq:mcc}
\end{equation}

Le MCC prend des valeurs dans $[-1, +1]$ : $+1$ correspond à une classification parfaite, $0$ à une performance équivalente à un classifieur aléatoire, et $-1$ à une classification parfaitement inversée. Contrairement à l'exactitude, au score $F_1$ ou à l'AUC-ROC pris isolément, le MCC est le seul coefficient qui prend en compte de manière symétrique toutes les cellules de la matrice de confusion, ce qui en fait le critère le plus informatif pour les problèmes à classes très déséquilibrées \citep{somvanshi2025survey}. La baseline XGBoost rapportée par \citet{azamuke2025rich} obtient MCC~$= 0{,}82$ sur les données MoMTSim.

\subsection{Aire sous la courbe ROC (AUC-ROC)}
\label{subsec:auc}

L'AUC-ROC mesure la capacité discriminante du modèle indépendamment du seuil de décision $\tau$. La courbe ROC trace le taux de vrais positifs (rappel) $\text{TPR} = \text{VP}/(\text{VP}+\text{FN})$ en fonction du taux de faux positifs $\text{FPR} = \text{FP}/(\text{FP}+\text{VN})$ pour tous les seuils $\tau \in [0,1]$. L'aire sous cette courbe est définie par la règle trapézoïdale appliquée aux $N$ seuils distincts :

\begin{equation}
    \text{AUC} = \int_0^1 \mathrm{TPR}\bigl(\mathrm{FPR}^{-1}(u)\bigr)\,\mathrm{d}u
    \approx \sum_{k=1}^{N} \frac{\mathrm{TPR}_k + \mathrm{TPR}_{k-1}}{2}
                                 \cdot \bigl(\mathrm{FPR}_k - \mathrm{FPR}_{k-1}\bigr)
    \label{eq:auc}
\end{equation}

L'AUC $= 1$ correspond à un classifieur parfait, l'AUC $= 0{,}5$ à une performance aléatoire. La baseline XGBoost rapportée par \citet{azamuke2025rich} obtient AUC~$= 0{,}97$. Cette métrique est recommandée par \citet{hou2024critical} comme indicateur de robustesse indépendant du seuil pour les architectures KAN.

\subsection{Précision, Rappel et score $F_1$}
\label{subsec:f1}

La précision mesure la proportion de transactions effectivement frauduleuses parmi celles classées comme frauduleuses par le modèle :
\begin{equation}
    \text{Précision} = \frac{\text{VP}}{\text{VP} + \text{FP}}
    \label{eq:precision}
\end{equation}

Le rappel (sensibilité) mesure la proportion de fraudes réelles détectées par le modèle :
\begin{equation}
    \text{Rappel} = \frac{\text{VP}}{\text{VP} + \text{FN}}
    \label{eq:rappel}
\end{equation}

Le score $F_1$ est la moyenne harmonique de la précision et du rappel, assurant un compromis équilibré entre détection (rappel) et fiabilité des alertes (précision) :
\begin{equation}
    F_1 = \frac{2 \cdot \text{Précision} \cdot \text{Rappel}}{\text{Précision} + \text{Rappel}}
    \label{eq:f1}
\end{equation}

Dans le contexte COBAC, cet arbitrage est directement lié au coût économique et réglementaire des erreurs : un Faux Négatif (fraude non détectée) entraîne des pertes financières directes, tandis qu'un Faux Positif (blocage d'une transaction légitime) génère des frictions pour les clients et des risques réputationnels \citep{gsma2024fraud}.

\subsection{Aire sous la courbe Précision-Rappel (PR-AUC)}
\label{subsec:prauc}

Pour les jeux de données fortement déséquilibrés, la courbe ROC peut être trompeuse car elle intègre les Vrais Négatifs, très nombreux, dans le calcul du Taux de Faux Positifs. La courbe Précision-Rappel (PR) constitue un outil plus discriminant pour les classes minoritaires \citep{Li2024} : elle trace la précision en fonction du rappel lorsque le seuil de décision varie de 1 à 0. L'aire sous cette courbe est estimée par la règle trapézoïdale :
\begin{equation}
    \text{PR-AUC} = \int_0^1 \text{Précision}(\text{Rappel}^{-1}(u))\,du
    \approx \sum_{k=1}^{N} \frac{\text{Précision}_k + \text{Précision}_{k-1}}{2}
    \left|\text{Rappel}_k - \text{Rappel}_{k-1}\right|
    \label{eq:prauc}
\end{equation}
où les points $(\text{Rappel}_k, \text{Précision}_k)$ sont obtenus en triant les exemples par score de fraude décroissant. Un PR-AUC élevé indique que le modèle maintient une précision satisfaisante même à haut rappel, condition nécessaire pour l'opérationnalisation dans un système d'alerte COBAC à volume élevé.

\subsection{Score de Brier}
\label{subsec:brier}

Le score de Brier mesure la calibration probabiliste d'un classifieur : il quantifie l'écart quadratique moyen entre la probabilité estimée $\hat{y}_i \in [0,1]$ et l'étiquette réelle $y_i \in \{0,1\}$ :
\begin{equation}
    \text{Brier} = \frac{1}{N}\sum_{i=1}^{N}\left(\hat{y}_i - y_i\right)^2
    \label{eq:brier}
\end{equation}
Un score de Brier nul indique une calibration parfaite ; le score maximum est 1 (classifieur inverse). Pour un modèle naïf prédisant toujours la fréquence marginale de fraude $\pi$, le score de Brier vaut $\pi(1-\pi)$. Dans un contexte COBAC où les décisions de blocage ont des conséquences réglementaires, la calibration est essentielle : une probabilité de 0,8 doit effectivement correspondre à 80~\% de transactions frauduleuses pour que le score puisse être utilisé comme seuil d'alerte pondéré \citep{gsma2024fraud}.

%======================================================
\section{Pipeline d'implémentation et d'entraînement}
\label{sec:pipeline}
%======================================================

Cette section documente les étapes techniques du pipeline MKAN tel qu'il est effectivement exécuté, depuis les fichiers \texttt{.parquet} bruts jusqu'au meilleur point de contrôle sauvegardé. Elle est complémentaire aux sections précédentes qui décrivent les fondements théoriques : ici, chaque choix d'implémentation est motivé par des contraintes pratiques d'environnement (GPU Intel Iris Xe via DirectML, 20~Go de RAM) et de volume (5,5~M de transactions en entraînement).

%--------------------------------------------------
\subsection{Stratégie de découpage et normalisation}
\label{subsec:split}
%--------------------------------------------------

Pour éliminer le biais de déplétion des comptes propre aux simulations de fraude séquentielles --- notamment le scénario ATO dont la prévalence décroît naturellement à mesure que les comptes compromis sont épuisés --- les trois partitions sont issues de \textbf{simulations MOMTSIM indépendantes}, chacune générée avec une graine distincte (tableau~\ref{tab:splits}).

\begin{table}[h!]
\centering
\caption{Partitions de données --- simulations indépendantes MOMTSIM}
\label{tab:splits}
\begin{tabular}{llrrrl}
\hline
\textbf{Partition} & \textbf{Seed} & \textbf{Clients} & \textbf{Transactions} & \textbf{Fraude} & \textbf{Rôle} \\
\hline
Train & 1000 & 510\,288 & 5\,569\,643 & 23{,}6~\% & Ajustement des paramètres \\
Val   & 1001 & 153\,159 & 1\,578\,746 & 19{,}1~\% & Sélection des hyperparamètres \\
Test  & 1002 & 153\,060 & 1\,579\,438 & 19{,}1~\% & Évaluation finale \\
\hline
\end{tabular}
\end{table}

\paragraph{Imputation des valeurs manquantes.}
Les features \texttt{delta\_commission} et \texttt{var\_agent\_split} présentent respectivement 5\,569\,643 et 4\,027\,625 valeurs manquantes dans l'ensemble d'entraînement. Ces NaN correspondent aux transactions pour lesquelles les indicateurs de smurfing et de fractionnement (éq.~3.13--3.14) ne sont pas définis. Ils sont remplacés par zéro avant tout traitement, valeur neutre sémantiquement correcte signifiant l'absence d'activité associée.

\paragraph{Transformations logarithmiques.}
Conformément au protocole du \texttt{TopologyValidator} (section~4.1), le test de Kolmogorov-Smirnov (éq.~4.5) est appliqué à chaque feature continue. Pour les features dont la distance $D_{KS} \geq 0{,}15$, la transformation suivante est appliquée :

\begin{equation}
    x'_{ij} = \log\!\left(1 + \max\!\bigl(x_{ij} - \min_{\mathrm{train}}(x_j),\; 0\bigr)\right)
    \label{eq:logtransform}
\end{equation}

Le décalage $\min_{\mathrm{train}}(x_j)$ est calculé \textit{exclusivement sur le train}, puis réappliqué à la validation et au test pour éviter toute fuite de données. Sur les 12 features MKAN, 9 dépassent le seuil ($D_{KS} \in [0{,}37\,;\,0{,}54]$) : seule $\rho_{\text{nouveau}}$ (éq.~3.19) est exemptée ($D_{KS} = 0{,}13$), les deux features binaires (\texttt{flag\_anomalie}, \texttt{flag\_nuit}) étant ignorées par construction \citep{hou2024critical}.

\paragraph{Normalisation z-score.}
Après transformation logarithmique, les features sont standardisées :

\begin{equation}
    \tilde{x}_{ij} = \frac{x'_{ij} - \mu_j}{\sigma_j + \varepsilon}
    \label{eq:zscore_impl}
\end{equation}

où $\mu_j$ et $\sigma_j$ sont la moyenne et l'écart-type de la feature $j$ calculés sur le train, et $\varepsilon = 10^{-6}$ assure la stabilité numérique. Les mêmes statistiques sont appliquées à la validation et au test, garantissant l'absence de contamination \citep{Li2024}.

%--------------------------------------------------
\subsection{Construction des fenêtres glissantes}
\label{subsec:windows}
%--------------------------------------------------

Conformément à l'équation~4.16, le modèle MKAN opère sur des séquences temporelles de transactions par compte. Pour chaque compte \texttt{nameOrig}, les transactions sont ordonnées par pas de temps \texttt{step}, puis toutes les sous-séquences consécutives de $W$ transactions sont extraites :

\begin{equation}
    \mathbf{X}^{(k)}_t = \bigl(\tilde{x}_{t-W+1}^{(k)},\; \tilde{x}_{t-W+2}^{(k)},\; \ldots,\; \tilde{x}_t^{(k)}\bigr) \in \mathbb{R}^{W \times d}
    \label{eq:window_impl}
\end{equation}

où $d = 12$ est la dimension des features et le label de la fenêtre est l'étiquette \texttt{isFraud} de la \textit{dernière} transaction $\tilde{x}_t^{(k)}$. Les comptes possédant moins de $W$ transactions sont exclus. Avec $W = 10$ pas de temps, le pipeline produit les volumes du tableau~\ref{tab:windows}.

\begin{table}[h!]
\centering
\caption{Volumes de fenêtres glissantes ($W = 10$, par partition)}
\label{tab:windows}
\begin{tabular}{lrr}
\hline
\textbf{Partition} & \textbf{Fenêtres} & \textbf{Taux de fraude} \\
\hline
Train & 1\,749\,109 & 43{,}9~\% \\
Val   &   450\,955  & 34{,}6~\% \\
Test  &   452\,349  & 34{,}6~\% \\
\hline
\end{tabular}
...
Le taux de fraude des fenêtres est sensiblement supérieur à celui des transactions brutes (23,6~\% pour le train). Ce phénomène s'explique par le mécanisme de fenêtrage : une transaction frauduleuse en fin de séquence se retrouve dans les $W - 1$ fenêtres précédentes du même compte, augmentant mécaniquement la proportion d'étiquettes positives \citep{Zhou2025}.

%--------------------------------------------------
\subsection{Construction des DataLoaders}
\label{subsec:dataloaders}
%--------------------------------------------------

Les tableaux NumPy issus de la fonction \texttt{make\_windows} sont convertis en tenseurs \texttt{float32} et encapsulés dans des \texttt{TensorDataset} PyTorch, puis exposés via des \texttt{DataLoader} configurés comme suit :

\begin{itemize}
    \item \textbf{Taille de lot} : 256 fenêtres par mini-lot, compromis entre la stabilité des gradients et l'occupation mémoire GPU.
    \item \textbf{Mélange} : activé sur le train (\texttt{shuffle=True}) pour briser les corrélations temporelles intra-lot ; désactivé sur la validation et le test pour la reproductibilité des métriques.
    \item \textbf{Travailleurs de chargement} : \texttt{num\_workers=0} --- le chargement est assuré par le fil principal afin d'éviter les conflits d'accès mémoire entre les processus de préchargement Python et le backend DirectML qui gère lui-même sa file de commandes GPU.
\end{itemize}

%--------------------------------------------------
\subsection{Recherche heuristique des hyperparamètres}
\label{subsec:heuristic_exec}
%--------------------------------------------------

La \texttt{RechercheHeuristique} (section~4.5) explore l'espace \texttt{ESPACE\_MKAN\_DEFAULT} composé de 7 hyperparamètres : la taille de l'état caché \texttt{hidden\_size}, le nombre de centres gaussiens $M$, l'ordre de Fourier $K$, la force de régularisation $\lambda$, les pondérations $\mu_1$ et $\mu_2$, et le taux d'apprentissage $\eta$. La taille de fenêtre $W = 10$ et la taille de lot sont fixes dans cette configuration.

\paragraph{Sous-échantillonnage pour l'évaluation fitness.}
Chaque évaluation de fitness consiste à entraîner un \texttt{MKANScorer} pendant $N_{\text{search}} = 10$ epochs et à calculer le MCC sur la validation. Avec 1\,749\,109 fenêtres d'entraînement, une évaluation complète serait prohibitive. Un sous-échantillonnage aléatoire reproductible (graine 42) réduit les volumes à $n_{\text{tr}} = 30\,000$ fenêtres train et $n_{\text{val}} = 10\,000$ fenêtres val, soit un facteur d'accélération d'environ $20\times$ par évaluation. Le budget total est borné par :

\begin{equation}
    B = N_{\text{gen}} \times T_{\text{pop}} \times N_{\text{search}}
      = 15 \times 12 \times 10 = 1\,800\;\text{epochs de recherche}
    \label{eq:budget}
\end{equation}

\paragraph{Paramètres de la recherche génétique.}
La configuration retenue est résumée dans le tableau~\ref{tab:rech_config}. La mutation adaptative s'inspire du refroidissement simulé \citep{haosol2024meta} : le taux de mutation $r_t$ décroît géométriquement selon $r_t = r_0 \cdot \rho^t$ avec $\rho = 0{,}97$, mais est réinitialisé lorsque la diversité de la population tombe sous le seuil de 0{,}40, afin de prévenir la convergence prématurée vers un optimum local.

\begin{table}[h!]
\centering
\caption{Configuration de la \texttt{RechercheHeuristique} (section~4.5)}
\label{tab:rech_config}
\begin{tabular}{ll}
\hline
\textbf{Paramètre} & \textbf{Valeur} \\
\hline
Générations ($N_{\text{gen}}$)      & 15 \\
Taille de population ($T_{\text{pop}}$) & 12 \\
Élite conservée                    & 2 individus \\
Taille du tournoi                  & 3 individus \\
Taux de mutation initial ($r_0$)   & 0{,}35 \\
Facteur de refroidissement ($\rho$) & 0{,}97 par génération \\
Seuil de diversité minimale        & 0{,}40 \\
Patience (arrêt anticipé)          & 5 générations sans amélioration \\
\hline
\end{tabular}
\end{table}

\paragraph{Warm start.}
À l'issue de la recherche, les poids du meilleur individu sont récupérés et rechargés dans le modèle final avant l'entraînement complet. Cette stratégie de démarrage à chaud (\textit{warm start}) évite de repartir d'une initialisation aléatoire et économise au minimum $N_{\text{search}}$ epochs supplémentaires \citep{Zhu2025}.

%--------------------------------------------------
\subsection{Boucle d'entraînement finale}
\label{subsec:training_loop}
%--------------------------------------------------

Le modèle \texttt{MKANScorer} est entraîné pendant $N_{\text{epoch}} = 40$ epochs sur l'intégralité des 1\,749\,109 fenêtres d'entraînement, avec les hyperparamètres sélectionnés par la recherche heuristique.

\paragraph{Optimiseur et écrêtage des gradients.}
L'optimiseur Adam est utilisé avec le taux d'apprentissage $\eta$ issu de la recherche (valeur de référence $\eta_0 = 10^{-3}$). Pour stabiliser la rétropropagation à travers les $W = 10$ pas de temps de la cellule T-KAN, les gradients sont écrêtés à la norme maximale 1 avant chaque mise à jour :

\begin{equation}
    \hat{g}_t = g_t \cdot \min\!\left(1,\; \frac{1}{\lVert g_t \rVert_2}\right)
    \label{eq:clipgrad}
\end{equation}

\paragraph{Fonction de perte et passage avant optimisé.}
La perte $\ell_{\text{total}}$ (section~4.3.4) est calculée à chaque mini-lot via \texttt{mkan\_total\_loss} avec $\lambda = 10^{-2}$, $\mu_1 = 1{,}0$, $\mu_2 = 0{,}5$. Les termes de régularisation $\sum_l |\Phi_l|_1$ et $\sum_l S(\Phi_l)$ sont accumulés sur les $W$ pas de temps au moyen de \texttt{forward\_with\_reg()}, qui calcule les activations d'arêtes $\phi_{ij}(x_i)$ une seule fois par porte et par pas de temps. Cette fusion divise par trois le nombre d'appels à \texttt{edge\_activations} par rapport à l'enchaînement naïf des trois opérations séparées (passage avant, norme L1, entropie), réduisant d'autant le coût du passage avant avec régularisation \citep{liu2024kan}.

\paragraph{Sélection du meilleur point de contrôle.}
À la fin de chaque epoch, le MCC est calculé sur l'ensemble de validation. Si ce score dépasse le meilleur observé, les poids courants sont sauvegardés :

\begin{equation}
    \theta^* = \underset{\theta_e,\; e \in \{1,\ldots,N_{\text{epoch}}\}}{\arg\max}\;
               \mathrm{MCC}\!\bigl(f_{\theta_e}(\mathcal{D}_{\text{val}})\bigr)
    \label{eq:bestckpt}
\end{equation}

Le MCC est préféré à la perte de validation comme critère de sélection car il est insensible au déséquilibre de classes (section~4.6.1) et directement aligné avec l'objectif opérationnel de détection de fraude \citep{azamuke2025rich}.
...
\bibitem[Azamuke et~al.(2024)]{azamuke2024momtsim}
...
\bibitem[Hou et~al.(2024)]{hou2024critical}
...
\bibitem[Wambugu et~al.(2024)]{gsma2024fraud}
...
\bibitem[Zhdanova et~al.(2014)]{zhdanova2014smurfs}
...
\bibitem[Azamuke et~al.(2025)]{azamuke2025rich}
...
Lu, Y., \& Zhan, F. (2024).
\textit{Kolmogorov--Arnold Networks in Fraud Detection: Bridging the Gap between Theory and Practice}.
Preprint. University of Cincinnati \& Stanford University. 5 septembre 2024.
...
\bibitem{azamuke2025dataset}
Azamuke, D., Katarahweire, M., \& Bainomugisha, E. (2025).
\textit{A labeled synthetic mobile money transaction dataset}.
Data in Brief, Elsevier.
\url{https://doi.org/10.1016/j.dib.2025.111474}

\bibitem{gsma2026state}
GSMA. (2026).
\textit{State of the Industry Report on Mobile Money 2025}.
GSMA Mobile for Development.
\url{https://www.gsma.com/sotir/}

\bibitem[Spall(1992)]{spall1992spsa}
Spall, J.~C. (1992).
Multivariate stochastic approximation using a simultaneous perturbation gradient approximation.
\textit{IEEE Transactions on Automatic Control}, 37(3), 332--341.

\bibitem{fu2002simulation}
Fu, M.~C. (2002).
Feature article: Optimization for simulation: Theory vs.\ practice.
\textit{INFORMS Journal on Computing}, 14(3), 192--215.

\bibitem[Law and Kelton(2000)]{law2000simulation}
Law, A.~M., \& Kelton, W.~D. (2000).
\textit{Simulation Modeling and Analysis} (3rd~ed.).
McGraw-Hill.

\bibitem[Azamuke et~al.(2022)]{azamuke2022scenario}
Azamuke, D., Katarahweire, M., \& Bainomugisha, E. (2022).
Scenario-based synthetic dataset generation for mobile money transactions.
In \textit{Proceedings of the Federated Africa and Middle East Conference on Software Engineering (FAMECSE '22)}.
ACM. \url{https://doi.org/10.1145/3531056.3542774}

\bibitem{gsma2010agents}
GSMA. (2010).
\textit{Managing a Mobile Money Agent Network: A Handbook for Mobile Network Operators}.
GSMA Mobile for Development.
\url{https://www.gsma.com/solutions-and-impact/connectivity-for-good/mobile-for-development/wp-content/uploads/2012/03/manage.pdf}

\bibitem{ifc2017liquidity}
International Finance Corporation / World Bank Group. (2017).
\textit{Liquidity Management for Mobile Money Providers}.
IFC Mobile Money Toolkit, Tool 10.5.
\url{https://documents1.worldbank.org/curated/en/802221501150875893/pdf/117459-WP-Tool-10-5-Liquidity-Management-Series-IFC-mobile-money-toolkit-PUBLIC.pdf}

\bibitem[Asongu and Odhiambo(2023)]{cogent2023agent}
Asongu, S.~A., \& Odhiambo, N.~M. (2023).
Agent liquidity: A catalyst for mobile money banking among the unbanked poor population in rural sub-Saharan Africa.
\textit{Cogent Economics \& Finance}, 11(1).
\url{https://doi.org/10.1080/23322039.2023.2203435}

\bibitem{gsma2018distribution}
GSMA. (2018).
\textit{Distribution 2.0: The Future of Mobile Money Agent Distribution Networks}.
GSMA Mobile for Development.

\bibitem[Fawcett \& Provost(1997)]{fawcett1997adaptive}
Fawcett, T., \& Provost, F. (1997).
Adaptive fraud detection.
\textit{Data Mining and Knowledge Discovery}, 1(3), 291--316.
\url{https://doi.org/10.1023/A:1009700419189}

\bibitem[Bolton \& Hand(2002)]{bolton2002statistical}
Bolton, R.~J., \& Hand, D.~J. (2002).
Statistical fraud detection: A review.
\textit{Statistical Science}, 17(3), 235--255.
\url{https://doi.org/10.1214/ss/1042727940}

\bibitem[Lopez-Rojas et~al.(2016)]{lopezrojas2016paysim}
Lopez-Rojas, E.~A., Elmir, A., \& Axelsson, S. (2016).
PaySim: A financial mobile money simulator for fraud detection.
In \textit{Proceedings of the 28th European Modeling and Simulation Symposium (EMSS 2016)}, pp.~249--255.
Larnaca, Cyprus.
\url{https://www.researchgate.net/publication/313138956}

\bibitem{deprez2025gargaml}
Deprez, B., Baesens, B., Verdonck, T., \& Verbeke, W. (2025).
...
Zhou, H., et al. (2025).
\textit{KAN-AD: Time Series Anomaly Detection with Kolmogorov-Arnold Networks}.
arXiv preprint.

\bibitem[Zhu et~al.(2025)]{Zhu2025}
Zhu, X., et al. (2025).
\textit{T-KAN: A Temporal Kolmogorov-Arnold Network for Time Series Forecasting}.
arXiv preprint.
